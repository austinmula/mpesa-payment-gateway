...
export class MpesaService {
  ...
  private async getAccessToken(): Promise<string> {
    try {
      ...
      const response: AxiosResponse<AccessTokenResponse> = await this.axiosInstance.get(
  ...
  async initiateSTKPush(paymentRequest: PaymentRequest): Promise<PaymentResponse> {
    try {
      ...
      const response: AxiosResponse<STKPushResponse> = await this.axiosInstance.post(
   ...
   */
  async querySTKPushStatus(checkoutRequestId: string): Promise<TransactionStatus> {
    try {
      const accessToken = await this.getAccessToken();
      const { password, timestamp } = this.generatePassword();
      ...
      const response: AxiosResponse<QuerySTKResponse> = await this.axiosInstance.post(
      ...
      return {
        ...
        errorMessage: status !== 'success' ? responseData.ResultDesc : undefined
      };
    } catch (error: any) {
  ...
  processSTKCallback(callbackData: STKPushCallback): TransactionStatus {
    try {
      ...
      const transactionStatus: TransactionStatus = {
        ...
        amount,
        phoneNumber,
        mpesaReceiptNumber: status === 'success' ? mpesaReceiptNumber : undefined,
